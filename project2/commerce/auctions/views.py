...
def listing_page(request, key):
    if request.method == "GET":
        ...
        return render(request, "auctions/listing_page.html",{
            ...
            "watchListItem": True if WatchList.objects.filter(user=request.user.username,listingId=key) else False
        })
    else:
        return HttpResponseRedirect(reverse("index"))
...
def removeFromWatchlist(request, key):
    if request.user.is_authenticated:
        try:
            watchlisting = WatchList.objects.get(user=request.user.username, listingId=key)
            watchlisting.delete()
            return HttpResponseRedirect(reverse('listing_page', args=(key,)))
        except:
            return HttpResponseRedirect(reverse('listing_page', args=(key,)))
    ...
    if request.method == "POST":
        ...
        if (entered_bid>current_bid):
            ...
            try:
                ...
                item_data.listingId = key
                item.save()
            response = HttpResponseRedirect(reverse('listing_page', args=(key,)))
